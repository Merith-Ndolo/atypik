...
const {
  ...
  validationErrors,
  signInAdminErrors,
} = require("../utilitaires/errors_utils");
...
module.exports.signUp = async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ pseudo, email, password, tel, role }, JWT_KEY, {
      expiresIn: "30m",
    });
    ...
    const mailOptions = {
      from: '"Atypikhouse G4" <nodejsa@gmail.com>', // sender address
    ...
    jwt.verify(token, JWT_KEY, (err, decodedToken) => {
      if (err) {
        ...
        UserModel.findOne({ email: email }).then((user) => {
          if (user.isValidate) {
            //------------ User already exists ------------//
            console.log("Compte déjà activé");
...
module.exports.signInPro = async (req, res) => {
  ...
  try {
    ...
    if (user.isValidate === false) {
      ...
      const errors = roleproErrors(user);
      res.status(200).json({ errors });
    } else {
      const token = creation_token(user._id);
      res.cookie("jwt", token, { httpOnly: true, temps_de_token });
    ...
    UserModel.updateOne({ resetLink: token }, (err, success) => {
      if (err) {
        ...
        const mailOptions = {
          from: '"Atypikhouse G4" <nodejsa@gmail.com>', // sender address
          to: email, // list of receivers
          subject: "Réinitialisation du mot de passe : NodeJS Auth ✔", // Subject line
          html: output, // html body
        };
